...
export const addPrediction = async (predictionData) => {
  try {
    ...
    if (!user) {
      return { success: false, error: 'User must be logged in to add predictions' };
    }
    ...
    const docRef = await addDoc(collection(db, 'predictions'), predictionWithMetadata);
...
export const getPredictions = async (filters = {}, pageSize = 10, lastDoc = null) => {
  try {
...
export const getPrediction = async (predictionId) => {
  try {
    ...
    if (docSnap.exists()) {
      return { success: true, prediction: { id: docSnap.id, ...docSnap.data() } };
    } else {
...
export const addBlogPost = async (postData) => {
  try {
    ...
    if (!user) {
      return { success: false, error: 'User must be logged in to add blog posts' };
    }
...
export const getBlogPosts = async (filters = {}, pageSize = 10, lastDoc = null) => {
  try {
...
export const createSubscription = async (subscriptionData) => {
  try {
    ...
    if (!user) {
      return { success: false, error: 'User must be logged in to create subscriptions' };
    }
    ...
    const docRef = await addDoc(collection(db, 'subscriptions'), subscriptionWithMetadata);
